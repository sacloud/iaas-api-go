...
func TestWebAccelOp_Find(t *testing.T) {
	...
	client := iaas.NewWebAccelOp(singletonAPICaller())
...
func TestWebAccelOp_Cert(t *testing.T) {
	...
	// create certs
	_, err = client.CreateCertificate(ctx, id, &iaas.WebAccelCertRequest{
	...
	// update certs
	certs, err := client.UpdateCertificate(ctx, id, &iaas.WebAccelCertRequest{
...
func TestWebAccelOp_DeleteAllCache(t *testing.T) {
	...
	// delete cache
	err = client.DeleteAllCache(ctx, &iaas.WebAccelDeleteAllCacheRequest{
		Domain: domain,
	})
	if !assert.NoError(t, err) {
		return
...
func TestWebAccelOp_DeleteCache(t *testing.T) {
	...
	// delete cache
	result, err := client.DeleteCache(ctx, &iaas.WebAccelDeleteCacheRequest{
		URL: urls,
	})
	if !assert.NoError(t, err) {
		return
