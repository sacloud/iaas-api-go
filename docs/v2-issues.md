...
### 3. `helper/{power, query, wait, plans, cleanup}` の v2 相当 ✅ 対応済み

**対応内容**
- `v2/helper/{wait, power, query, cleanup, plans}` を新設し、v1 `helper/*` の主要
  関数を v2 型 (`int64` ID / ogen 生成型) で再実装した。`helper/api` は saclient-go
  + api-client-go でカバー済みのため port しない。
- v2 → v1 依存は行わない (v1 `iaas.*` / `types` / `search` / `ostype` を import しない)。
  `ostype.ArchiveOSType` と criteria は v2 helper 内で自前定義した。
- 各 Until* / Delete* / Is*Referenced などは **narrow interface** を引数に取り、
  テスト fake を書きやすく、モノレポ切り出し時の依存も最小化している。
- 主要 API:

| パッケージ | 代表関数 |
|---|---|
| `wait` | `StateWaiter` / `SimpleStateWaiter` / `UntilServerIs{Up,Down}` / `UntilApplianceIs{Up,Down,Ready}` / `UntilArchiveIsReady` / `UntilDiskIsReady` |
| `power` | `BootServer` / `ShutdownServer` / `BootAppliance` / `ShutdownAppliance` (still_creating retry + 逆方向状態時の再送ロジックを含む) |
| `query` | `FindArchiveByOSType` (v2 内 `ArchiveOSType` enum) / `FindServerPlan` / `ReadServer` / `ReadRouter` (previous-id fallback + `ErrNoResults`) / `Is{Disk,CDROM,Switch,Bridge,PrivateHost,PacketFilter,SIM}Referenced` + `WaitWhile*IsReferenced` |
| `cleanup` | `DeleteServer` (withDisks) / `DeleteDisk` / `DeleteCDROM` / `DeleteSwitch` / `DeleteBridge` / `DeletePacketFilter` / `DeletePrivateHost` / `DeleteSIM` / `DeleteInternet` / `DeleteMobileGateway` |
| `plans` | `AppendPreviousIDTagIfAbsent` / `ChangeServerPlan` / `ChangeRouterPlan` |

**テスト**
- unit test 57 件 (fake reader / fake op ベース、hardcoded timing を短縮してループ検証)
- ACC test: `v2/integration/helper_{wait,power,query,cleanup,plans}_test.go`
  (`TEST_ACC=1` 必須、`SAKURA_TRACE=1` でトレース可)

**既知の制約 (v2 spec 未対応で省略した部分)**
- `IsSwitchReferenced`: v1 では Appliance 系 (DB/LB/NFS/MGW/VPCRouter) の Interface
  走査で Switch 参照を検出していたが、v2 spec の Database 系レスポンスに Interface ごとの
  Switch 情報が無いため Switch.GetServers 経由の検査のみ。
- `Switch.HybridConnectionID` も v2 spec 未公開のため参照検査から除外。
- `ReadProxyLB`: v2 では ProxyLB が CommonServiceItem 経由になる一方、
  `CommonServiceItemOpRead` の envelope が AutoBackup 固定型のため ProxyLB fat model
  を返せず、いったん実装見送り。downstream で必要になった時点で再設計する。

これらは v2 spec 側の追加 (`InterfaceSettings` 内に Switch 情報を露出、ProxyLB 専用
envelope の追加など) と合わせて後続で拡張する。

**今後**
- モノレポ化のタイミングで `v2/helper/` を `github.com/sacloud/helper-go` 相当の
  独立パッケージへ配置換えする (SEG 等他サービスからも共用)。
...
メンテナンス性は高い。最優先 4 項目 (#1〜#4) は方針決定済で、#1 / #4 は実装完了、
#2 は一次対応済 (`OptNilResourceRef` の段階縮小を継続)、#3 は helper port の実装が未着手。

推奨する作業順序:

1. #3 helper 群の v2 port（`power` → `cleanup` → `wait` → `query` → `plans` の順）。
   terraform-provider-sakuracloud の v2 移行と並行で進められる。
