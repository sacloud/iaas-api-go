...
### 1. `iaas.IsNotFoundError` が v2 に無い

**状況**
- downstream の使用実態 (2026-04-23 時点):
  - `IsNotFoundError` … terraform-provider-sakuracloud + usacloud で 78 箇所
  - `IsNoResultsError` … 3 箇所
  - `IsStillCreatingError` … downstream 0 箇所（v1 `helper/power.BootRetry` 内部のみで使用）
- v2 `error.go` は `saclient.NewError` に委譲しているため、`saclient.IsNotFoundError(err)` は
  `errors.As` 経由で機能する。ただし `iaas.` 名前空間に wrapper が無い。

**影響**
- `IsNotFoundError` の 78 箇所を `saclient.IsNotFoundError` に package rename するだけで済むが、
  downstream 側 PR の diff が肥大する。
...
- `v2/error.go` に 1 行追加:
  ...
  ```
  → 78 sites の package rename を回避できる。メンテ負荷ゼロ。
- `IsStillCreatingError` は **追加しない**。downstream 未使用。helper/* を v2 化する段階 (#3) で
  必要なら helper パッケージ内部に閉じて実装する。
- `IsNoResultsError` は **追加しない**（概念ごと削除）。v2 の Find ops は空リストを返すだけで
  `NoResultsError` 相当を吐かないため、v2 で定義しても常に false になり誤解を招く。downstream
  3 箇所は「結果が空か」のチェックに移行時に書き換える。
- 細かい条件判定 (`error_code == "still_creating"` 等) は #12 の `*iaas.Error` アクセサ実装で
  `err.Code() == "still_creating"` のように書けるようにする。
...
- #1 で `IsStillCreatingError` / `IsNoResultsError` を追加しない方針にしたため、これらの
  条件判定は downstream で `err.Code() == "still_creating"` のように書くことになる。
  アクセサ実装が揃っていないと downstream 側で `errors.As` のボイラープレートが増える。
...
- v1 `APIError` interface と同名・同シグネチャにすれば downstream の移行が機械的になる。
