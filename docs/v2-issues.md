...
### 3. `helper/{power, query, wait, plans, cleanup}` の v2 相当 ✅ 方針決定済

**方針**
- v2 は **API client としての薄いラッパー** という位置づけを明示。
- v2 → v1 の依存は**原則行わない**（v2 から v1 `iaas.*` / `types` を import しない）。
- helper は **原則 port しない**。**頻出 AND 実装重め** のケースが downstream 移行で
  実痛として観測されたとき、v2/helper/ へ個別に port する。

**各 helper の判定**

| helper | 判定 | 根拠 |
|---|---|---|
| `api` | port しない | `saclient-go` + `api-client-go` が env/profile 読み込みを提供。`v2/client.go` で組み込み済み。 |
| `plans` | port しない | 164 LOC、downstream は 2 箇所/repo。前 ID タグ操作は downstream inline で十分（~30 LOC）。 |
| `query` | port しない | 1,333 LOC の大半は `FindByOSType` 等の薄いラッパー。downstream は `ostype` + v2 ogen Find の直接呼び出しで代替可能。参照チェック系 (`IsXxxReferenced`) に需要が出たら個別判断。 |
| `wait` | watch list | v2/saclient に polling 無し。usacloud で `UntilServerIsUp` 等が heavy に使われるため、downstream が polling を自前実装する段階で再評価。 |
| `power` | watch list | `still_creating` retry + force shutdown fallback は重い。terraform で `ShutdownServer` / `ShutdownDatabase` が頻出。downstream 移行で retry ロジックの書き直しが痛いと判明したら port 検討。 |
| `cleanup` | watch list | 参照チェック + 順序付き削除のドメインロジック。terraform `Delete` handler で頻出。downstream 実装で分量が肥大したら port 検討。 |

watch list の 3 つは「port する予定」ではなく「port 候補として記憶しておく」。
トリガーが来るまでは downstream 側 inline / v1 継続のいずれかで回す。

**新規 helper port 提案の判断基準（future 用）**
以下すべてを満たす場合のみ v2/helper/ に追加:
1. downstream ≥2 リポジトリで使用頻度が高い（>= 5 callsite/repo 目安）
2. 実装が非自明（retry / polling / state machine / 参照チェック等のドメインロジックを含む）
3. v2 client / saclient-go / ogen 生成物で賄えない
4. v1 の API 仕様ではなく、v2 の API/型で再実装可能
...
メンテナンス性は高い。最優先 4 項目 (#1〜#4) は方針決定・一次対応を完了しており、
残る継続課題は #2 の `OptNilResourceRef` 追加対応 (観測ベースでの段階縮小) と、
watch list 扱いの helper port トリガー監視のみ。

推奨する作業順序:

1. 高 5〜7（test / middleware / ogen config）を整理し、v2 公開前提の線を固める
2. 生成器側（中 8〜10）は後回しでよい。面積が広がっても移行ブロッカーにはならない
3. ドキュメント（低 14〜17）はリリース判断前にまとめて更新
4. downstream 移行を進める中で #3 watch list (power/cleanup/wait) が閾値を超えたら個別 port
