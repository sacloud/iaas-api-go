...
### 3. `OptNilString` / `OptNilInt64` 氾濫の ergonomics

**状況**
- TypeSpec が全スカラーフィールドを `Name?: string | null` として出力 → ogen が
  `OptNilString`（`.Set` / `.Null` / `.Value` の 3 段アクセス）を生成。
- 例: v2/integration/ 配下で `.Value` が 166 回、`fmt.Sprintf("%d", id)` が 51 回出現。
  downstream では更に悪化する。
- `fieldNullabilityOverrides` は「nullable 化」方向にのみ使われており、反対方向のルール
  （non-nullable / required 化）が未整備。

**影響**
- ボイラープレートの爆発。downstream の読み書き全行に `.Value` が付く。
- null と zero 値の意味論が曖昧なフィールドでもすべて 3 段 unwrap が強制される。

**対応案**
- fieldmanifest と並列に「null で返らないことが確認できたフィールドを non-nullable に格下げする」
  allowlist を追加（候補名: `fieldNonNullableOverrides`）。
- 実測（integration テストで観測した値）とマニュアル記述を根拠にして、`Name` や `Tags` など
  常に値が入るフィールドから順に落とす。
- 変更時の回帰は verify-typespec に「このフィールドは OptNilString にならず string のまま」チェックを追加。
...
**影響**
- 公開 API 面の実ユースが常に 1 パスしかテストされない。
- ラッパー層でのリグレッションが気付かれない（raw ogen 側は通っても wrapper 側が壊れるケース）。

**対応案**
- `newClient` を `iaas.NewClient` + op ラッパー経由に統一。
- raw ogen を呼びたいテストだけ `*client.Client` を別ヘルパで露出。
- 移行完了後、`helper_test.go` の `securitySource` / `baseTransport` は削除できる。
