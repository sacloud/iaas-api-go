...
**注意**: 公式マニュアル（https://manual.sakura.ad.jp/cloud-api/1.1/index.html）には **202 Accepted**（受付完了・処理未完了）の定義もある。現在は 201/200 の決め打ちで運用中だが、非同期処理系のエンドポイントで 202 が返ると ogen の decode が失敗する可能性あり。遭遇したら generator 側で両対応を検討する。
