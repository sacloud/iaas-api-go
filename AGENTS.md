...
Go DSL 定義
  → TypeSpec 生成 (gen-typespec)
      ※ fieldmanifest allowlist / excludedOps で downstream 未使用を除外
  → OpenAPI 生成 (tsp compile)
  → Go クライアント生成 (ogen)
  → v2 ラッパー生成 (gen-v2-op)
```

`cd spec && pnpm run build` で TypeSpec → OpenAPI → Go クライアントまでが実行される。

```
pnpm run build
  = generate                # DSL → TypeSpec ファイル生成 + docs/typespec/excluded-fields.md 更新
  + verify                  # 生成物の退行検査（internal/tools/verify-typespec）
  + format                  # tsp format
  + lint                    # TypeSpec → OpenAPI (tsp-output/@typespec/openapi3/openapi.yaml)
  + generate:client         # OpenAPI → Go クライアント (v2/client/)
  + generate:find-request   # findmanifest → v2/client/find_request_gen.go
```

`v2/<resource>_gen.go`（ogen invoker のラッパー）は別途 `go run ./internal/tools/gen-v2-op/` で生成する。`excludedOps` で bucket が空になった場合の stale ファイルは自動削除される。
...
- `fieldNullabilityOverrides` で nullable 化した一部 submodel (SwitchUserSubnet 等) — `skipIfModelAbsent: true` 付きで、fieldmanifest allowlist で emit されなくなった場合はスキップ
...
| `models.go` | `resources/{name}/models.tsp` を生成。`fieldmanifest` allowlist も適用 |
| `ops.go` | `resources/{name}/ops.tsp` を生成（共有グループは `resources/{group}/ops.tsp`）。`excludedOps` を適用 |
...
| `excluded_ops.go` | 生成対象から除外する DSL op のマップ。詳細は「downstream 参照基準での field / op pruning」参照 |
| `exclusions_report.go` | fieldmanifest で除外されたフィールドを `docs/typespec/excluded-fields.md` に書き出す |

### 関連パッケージ

| パッケージ | 役割 |
|---|---|
| `internal/tools/fieldmanifest/` | downstream で参照される TypeSpec フィールドの allowlist。`gen-typespec` が参照 |
| `internal/tools/findmanifest/` | Find 系 `?q` クエリで受け付けるフィルタキーの allowlist。`gen-find-request` / `gen-typespec` が参照 |
| `internal/tools/gen-find-request/` | allowlist を元に `v2/client/find_request_gen.go`（Find 用 typed request / filter 構造体）を生成 |
| `internal/tools/gen-v2-op/` | ogen 生成の invoker メソッドをラップした `v2/<resource>_gen.go` を生成。op が 0 件の bucket の stale ファイルは自動削除する |

### 生成物検証（`internal/tools/verify-typespec/`）

`main.go` — 生成された `spec/typespec/resources/*/*.tsp` を読んで、特別対応した model / envelope が期待フィールドを保っているかをチェックする。ビルドパイプラインの `verify` ステップから呼ばれる。`skipIfModelAbsent: true` を指定したチェックは、モデル自体が `fieldmanifest` allowlist で emit されなくなった場合に自動でスキップされる。
...
**例外的に生成から外したい op**: `excluded_ops.go` の `excludedOps` マップで TypeSpec 生成から除外する (詳細は下記「downstream 参照基準での field / op pruning」節)。

### downstream 参照基準での field / op pruning

**問題**: v1 DSL は「v1 が過去に実装してきた全フィールド／全 op」の集合であり、v2 TypeSpec / v2 Go client にそのまま引き出すと面積が膨らむ。実際に downstream (usacloud / terraform-provider-sakuracloud / terraform-provider-sakura) と iaas-service-go が読み書きしているのは一部のフィールド / エンドポイントに過ぎない。

**方針**: v1 DSL は無改変で、v2 生成段階で「実際に downstream が参照するもの」だけに絞る。絞る対象は以下 2 レイヤー。

#### 1. フィールド単位の allowlist — `internal/tools/fieldmanifest/manifest.go`

- キー = TypeSpec モデル名、値 = emit する TypeSpec フィールド名のセット
  ```go
  "Archive": {
      "ID": true, "Name": true, "Description": true, "Tags": true,
      "Icon": true, "SizeMB": true, "Availability": true,
  },
  ```
- 未登録モデルは **pass-through**（全フィールドを emit）。段階的な導入を許す
- TypeSpec 側の名前で判定する（mapconv リネーム後の名前。例: v1 `IconID` → v2 `Icon`）
- 同一 TypeSpec モデルが response / request の両方で参照されるケース（例 `DatabaseRemarkDBConfCommon`）は **union** を登録すること
- 適用タイミング: `models.go` の `emitFromFieldTree` で `modelFieldExclusions` (denylist) の直後
- 除外されたフィールドは `docs/typespec/excluded-fields.md` に自動レポート出力される（`exclusions_report.go`）

#### 2. オペレーション単位の除外 — `internal/tools/gen-typespec/excluded_ops.go`

- キー = API 名、値 = 除外する op 名のセット
  ```go
  "VPCRouter": {"Logs": true},
  "LoadBalancer": {"MonitorCPU": true, "Status": true},
  ```
- 除外した op が参照していた model / envelope も、他 op から参照されなければ連動して emit されなくなる（例: `VPCRouter.Logs` 除外で `VPCRouterLog` / `VPCRouterLogsResponseEnvelope` が消える）
- 単一リソース `ops.tsp` と共有グループ `ops.tsp`（resource-unique op の列挙部）の両方で `opIsExcluded` を評価する

#### 「使われている」の判定基準

- **直接参照**: usacloud / terraform-provider-sakuracloud (v2) / terraform-provider-sakura (v3)
- **間接参照**: iaas-service-go は将来廃止予定なので完全網羅はしない。ただし上記 3 downstream が iaas-service-go 経由で使っているシンボル（とその iaas 型参照フィールド）は保持する
- **v2/integration テストも consumer 扱い**: Availability polling 等、実 API 挙動の検証に必要なフィールドは保持。テスト専用と判断できるものはテスト側を書き換える

#### 新フィールド / op 追加・除外変更フロー

- downstream が新フィールドを使い始めた → `fieldmanifest.Manifest` にエントリ追加 / 更新
- 新 op を v2 に出したくなった → `excludedOps` から該当エントリを除去
- いずれも `go run ./internal/tools/gen-typespec && cd spec && pnpm run build && go run ./internal/tools/gen-v2-op` で再生成
- op 除外で bucket が空になった場合の stale `v2/<name>_gen.go` は `gen-v2-op` が自動削除する
- `modelFieldExclusions`（denylist）と `fieldNullabilityOverrides`（nullability 調整）は fieldmanifest とは直交した責務。fieldmanifest で削られたモデルを参照する nullability override は残っていても無害だが、`verify-typespec` のチェックには `skipIfModelAbsent: true` を付けて「モデルごと消えた場合はチェックもスキップ」できるようにしている

### Optional と Nullable の分離（`?` と `| null`）

**方針**: TypeSpec で `?`（optional） と `| null`（nullable）は **別概念**として扱う。

| ケース | TypeSpec 出力 | ogen 出力 | 意味 |
|---|---|---|---|
| naked で `*T`（ポインタ） | `Foo?: T \| null` | `OptNilT` | レスポンス/リクエストとも null 許容 |
| naked で `T` + `json:",omitempty"` | `Foo?: T` | `OptT` | レスポンスに無い可能性／リクエスト側で省略可 |
| naked で `T`（omitempty なし） | `Foo: T` | `T` | 必須 |
| `fieldNullabilityOverrides` エントリ | `Foo?: T \| null` | `OptNilT` | 実測で null 確認済み |

`models.go` の `nakedFieldIsNullable` は **ポインタ型 `*T` のときのみ true** を返す。`omitempty` は optional（`?`）のシグナルとしてのみ使い、`| null` は付けない。

**背景**: 以前は `omitempty` も nullable 扱いしていたため、`Name` のように実 API では必ず値が入るフィールドまで `OptNilString`（`.Set` / `.Null` / `.Value` の 3 段アクセス）になっていた。
downstream の読み書き全行に `.Value` が付く ergonomics 劣化が [`docs/v2-issues.md` 項目 3](./docs/v2-issues.md) として記録されていた。
方針として「GET レスポンスは実測で null が返ると確認できたフィールドのみ nullable とする」に揃え、`omitempty` から自動 `| null` する挙動を廃止した。

**実測で null が返るフィールドの追加フロー**:

1. 統合テストで `null` 由来の decode 失敗（`invalid: XXX (field required)` など）に遭遇する、もしくは SAKURA_TRACE で `"FieldName": null` を確認する
2. `models.go` の `fieldNullabilityOverrides` に `"ModelName": {"FieldName": true}` を追加（コメントに実測根拠を残す）
3. `verify-typespec` に対応するチェックを追加（任意）

pointer 由来の nullable（v1 DSL が `*T` と宣言していたもの）は現状維持。将来的にはこれも実測ベースの override に移行したいが、スコープ外。
...
| Appliance | GET `/appliance/:id/power` | 電源状態取得 | v1 未実装（view の `Instance.Status` で取得可） |
| Appliance | GET `/appliance/:id/interface/:index` | NIC 詳細取得 | v1 未実装（view の `Interfaces` で取得可） |
| Appliance | GET `/appliance/:id/cpu/:serverindex/monitor` | 個別サーバ CPU モニター | v1 未実装（Appliance は単一インスタンス前提なので `/cpu/monitor` のみ使う） |
| Appliance | GET `/appliance/:id/disk/:diskindex/monitor` | 個別ディスクモニター | v1 未実装（Database 用に `/disk/0/monitor` 固定で実装済） |
| Appliance | GET `/appliance/:id/nfs/:serverindex/monitor` | 個別 NFS モニター | v1 未実装 |
| Appliance | GET `/appliance/:id/nfs/monitor` | NFS モニター | v1 未実装（NFS は `/interface/monitor` を利用） |
| Appliance | GET `/appliance/:id/server/:serverindex/disk/:diskindex/monitor` | サーバ配下ディスクモニター | v1 未実装 |
| Appliance | PUT `/appliance/:id/server/:serverindex/plan` | 個別サーバのプラン変更 | v1 未実装（`PUT /appliance/:id/plan` 側を使用） |
| Appliance | PUT `/appliance/:id/interface/:index/to/switch/shared` | 共有セグメント接続 | v1 未実装（MobileGateway は専用 `/mobilegateway/...` 経由） |
| Database | GET / PUT `/appliance/:id/database/:serverindex/monitor` | 個別 DB モニター | v1 未実装 |
| Database | GET / PUT `/appliance/:id/database/plugin` | Database プラグイン管理 | v1 未実装 |
| Database | GET `/appliance/:id/database/replica-slaves` | レプリカスレーブ一覧 | v1 未実装 |
| Database | GET `/appliance/:id/database/slaves` | スレーブ一覧 | v1 未実装 |
| Database | GET `/appliance/:id/database/slaves/status` | スレーブ状態取得 | v1 未実装 |
| Database | GET / PUT `/appliance/:id/database/syslog` | Database syslog 設定 | v1 未実装 |
| Database | DELETE `/appliance/:id/database/parameter` | DB パラメータ削除 | v1 は GET / PUT のみ実装 |
| VPCRouter | GET `/appliance/:id/vpcrouter/sitetosite/connectiondetails` | サイト間 VPN 接続詳細 | v1 未実装 |
| VPCRouter | PUT `/appliance/:id/vpcrouter/restartvpn` | VPN 再起動 | v1 未実装 |
| CommonServiceItem | PUT `/commonserviceitem/:id/plan`（汎用） | プラン変更（汎用パス） | v1 は ProxyLB 用のみ実装。他リソース横断の汎用版は未対応 |
| CommonServiceItem | GET `/commonserviceitem/:id/activity/awsdirectconnect/monitor` | AWS Direct Connect モニター | v1 未実装（downstream 利用なし） |

### マニュアル記載のうち TypeSpec 対象外の事項

公式マニュアルには記述があるが、API スキーマではなくドキュメント読み物としての情報のため TypeSpec には載せない（載せても OpenAPI / ogen の生成には寄与しない）。監査の際にこれらを「漏れ」として再掲しないよう明示しておく。

- **認証方式**：Basic / Digest、API キー・シークレットトークン取得手順
- **リクエスト/レスポンス形式**：HTTPS 必須、JSON / UTF-8、ISO 8601 の日時（例 `2013-01-14T09:30:00+09:00`）
- **`X-Sakura-API-Beautify: 1` ヘッダ**（整形出力。実運用では未使用）
- **ゾーン URL 一覧**：`tk1a / tk1b / is1a / is1b / is1c / tk1v`（TypeSpec は `{zone}` パスパラメータ化まで）
- **HTTP ステータスコード一覧の意味**（200〜503）。POST のコード方針は「HTTP ステータスコード」節で別途規定済み
- **共通クエリ仕様の解説**：`From / Count`、`Sort` 降順記法、フィルタ AND、`Include / Exclude`、ワイルドカード。v2 は `?q={json}` を `@query q` として通すのみで、内部構造の文書化はしない
- **レスポンス構造の解説**：`Total / From / Count / is_ok` 等の envelope 仕様（本書「レスポンスエンベロープの共通フィールド」で独自方針を記述済み）
- **運用ガイド**：集中アクセス回避等のベストプラクティス
- **列挙型の日本語説明**：`EAvailability / EDiskConnection / EDiskEncryptionAlgorithm / EInterfaceDriver / EServerInstanceStatus / EHostClass / ECPUModel / EHostCommitment / EStorageClass / EApplianceClass / ENoteClass / EScope` 等の用途解説。TypeSpec 側は enum 値の列挙のみ
- **変更履歴（`changes/`）**
...
```bash
cd v2
TEST_ACC=1 go test -v -timeout 30m ./integration/...
```

Appliance 系（Database/LoadBalancer/NFS/VPCRouter）は 1 ケースあたり 1〜5 分かかるため、
デフォルトの 10 分タイムアウトでは全量流し切れない。`-timeout 30m` 以上を必ず指定する。

累積した test リソースを掃除したいときは `TEST_ACC_CLEANUP=1` を併用する。`TestCleanup*` テストが
"test"/"integration" タグ付きリソースを削除する（一部は tk1a 固定）。
